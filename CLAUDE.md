...
**PR のマージはユーザーが行う。Claude は `gh pr create` で PR を作成し URL を案内したら停止すること。`gh pr merge` は実行しない。**

---

## 6. サーバー起動ルール（ポート競合時の対処）

サーバー起動時にポートが競合した場合、**別のポートで起動することは絶対に禁止**。
必ずアプリで指定されているデフォルトポートで起動すること。
...
```bash
# 競合しているプロセスの PID を特定する
netstat -ano | findstr :<PORT>

# 該当プロセスを強制停止する
taskkill /PID <PID> /F

# 停止を確認してから指定ポートでサーバーを起動する
```

### 禁止事項
- `--port 8081` など代替ポートを指定して起動すること
- ポート競合をそのままにして別ポートで動作確認すること
...
7. [ ] `gh pr create` で PR を作成し、URL をユーザーに案内して停止する
8. [ ] （ユーザーが）PR をマージし、ブランチを削除する
